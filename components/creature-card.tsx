...
import CreatureStatsDialog from "./creature-stats-dialog";
...
export default async function CreatureCard({
  ...
  if (!creature) {
    return (
      <div className="grid w-full place-content-center gap-5 bg-background px-4 text-center">
        ...
        <Link
          href={serializeFormSearchParams("/", { username: username })}
        >
  ...
  const theme = getPowerLevelTheme(cardTheme);
  const emblemTheme = getEmblemTheme(cardEmblemTheme);

  const toggleId = `stats-toggle-${username}`;
